    ...
    <aside className="rounded-lg border border-outline-variant bg-surface-container p-lg xl:sticky xl:top-24">
      ...
      <div className="space-y-sm">
        <Button className="h-11 w-full bg-primary text-on-primary hover:bg-primary/90" disabled={isSubmitting} type="submit">
          {isSubmitting ? "Posting..." : requestType === "instant" ? "Find Instructor Now" : "Post Request"}
        </Button>
        <Button
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await createRequest({
        ...
        discount_per_extra_student: form.discountPerStudent ? Number.parseFloat(form.discountPerStudent) : undefined,
        minimum_price: form.minimumPrice ? Number.parseFloat(form.minimumPrice) : undefined,
        final_price_per_student: form.budget ? Number.parseFloat(form.budget) : undefined,
        max_students: form.sessionMode === "group" && form.maximumStudents ? Number.parseInt(form.maximumStudents, 10) : undefined,
      });
