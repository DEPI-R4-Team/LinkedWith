      ...
      <div className="flex flex-wrap items-center gap-sm">
        ...
        {showOptions ? (
          <div className="absolute right-lg top-20 z-20 w-52 rounded-md border border-outline-variant bg-surface-container p-xs shadow-lg">
            <Link className="block rounded-md px-sm py-xs text-body-sm text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface" to={sessionPath}>
              View Session
            </Link>
            <Link className="block rounded-md px-sm py-xs text-body-sm text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface" to={profilePath ?? `/student/instructors/${conversation.id}`}>
